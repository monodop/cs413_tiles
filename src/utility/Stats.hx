...
class Stats extends Sprite {	
	...
	private function update(e : Event) {
		...
		if( timer - 1000 > ms_prev ) {
			...
			xml.fps = new XMLList("FPS: " + (Math.round(fps * 100) / 100) + " / " + stage.frameRate);
			xml.mem = new XMLList("MEM: " + (Math.round(mem * 100) / 100));
			xml.memMax = new XMLList("MAX: " + (Math.round(mem_max * 100) / 100) );
